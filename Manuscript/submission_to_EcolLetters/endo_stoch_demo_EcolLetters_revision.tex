...
These inherent characteristics include life history traits \cite{pfister1998patterns, morris2008longevity}, negative correlations among vital rates \cite{compagnoni2016effect}, transient shifts in population structure \cite{ellis2013role}. 
...
Intra-specific interactions, such as parental care, can reduce demographic variance in birds  \cite{dybala2013dependent} and in primates \cite{colchero2021evidence}.
While most research on inter-specific interactions focuses on their effects on mean demographic performance, species interactions also have the potential to influence demographic variance.
...
\section*{Materials and Methods}
	...
	Using the Rage package \cite{jones2022rcompadre}, we calculated $R_0$, longevity, and generation time from the mean transition matrix for symbiont-free populations.
...
%	For each contribution element (variance buffering, mean effects, and their interaction), we calculated a cross-species mean to assess the overall contributions (Fig. 4).
	...
	By sampling away from an average year in both directions, the six- and two- years scenarios increased the standard deviation of yearly host growth rates by $1.3$ and $2.1$ times, respectively, without changing mean growth rates ($<2.3$\% difference in $\overline{\lambda}$ between simulation treatments, Fig. S22).
...
Across the 14 census years, endophytes reduced inter-annual variance for 66\% ($37$/$56$) of host species-vital rate combinations (average Cohen's D for effects on vital rate standard deviation: $-0.15$) (Fig 1A; Fig. S6 - Fig. S18). 
...
The magnitude of mean and variance effects differed among host species and vital rates. Symbiont effects on vital rate variance are as large and even exceed effects on vital rate means for certain species.
For example, endophytes modestly increased mean adult survival (Fig. 1C) and strongly reduced variance in survival (Fig. 1D) for \emph{Festuca subverticillata}, while for \emph{Poa alsodes}, variance buffering was more apparent in seedling growth and inflorescence production (Fig 1E). 
...
Similarly, variance was increased for certain species' vital rates, such as in seedling growth for \emph{Elymus villosus} and \emph{Festuca subverticillata} (Fig. 1A).
...
\josh{Maybe need to bring up the "prediction" here or in introduction, but I moved it to the discussion}{}


In support of the prediction that hosts with long lifespans, which are intrinsically buffered from environmental variability, should benefit less from symbiont-mediated variance buffering, we found that hosts with trait values representing faster life history strategies experienced greater variance buffering from endophytes than those with slow life histories (Fig. 3).
...
To evaluate the relative importance of mean fitness benefits and variance buffering as alternative pathways of mutualism, we decomposed  the overall effect of the symbiosis on the stochastic growth rate $\lambda_{S}$ using simulations from the population models in \josh{four configurations}{Could update this to include the VR decomposition}.
...
These alternative strategies may to the long-term stability of these mutualism
...
To forecast how symbionts may affect hosts in future climates across their ranges, we need to consider that this is a field experiment in one location.
...
To DO: remake size-struct plot for each species

Remake size structure for only recruit plantsecoles

- The fact that this is a field experiment in one location


SPEI analysis? and climate; implicit to explicit treatment of drivers; types of envrionemtnal variability


- Evolutionary context: i.e. different alternative strategies of mean and variance; diverse microbiome, bet-hedging
