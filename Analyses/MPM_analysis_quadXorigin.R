...
saveRDS(lambda_mean, file = "~/Dropbox/EndodemogData/Model_Runs/MPM_output/lambda_mean_quadXorigin.rds")
...
saveRDS(lambda_var, file = "~/Dropbox/EndodemogData/Model_Runs/MPM_output/lambda_var_quadXorigin.rds")
...
summarylambda_join_df <- lambda_join_df %>% 
  ...
  summarize(avg_meandiff = median(meanlambda_diff,na.rm = TRUE),
            avg_vardiff = median(varlambda_diff, na.rm = TRUE))
...
meanvar_biplot <- ggplot(data = lambda_join_df) +
  ...
  geom_point(data = summarylambda_join_df, aes(x = avg_meandiff, y = avg_vardiff, color = species), size  = 3) +
  geom_label_repel(data = summarylambda_join_df, aes(x = avg_meandiff, y = (avg_vardiff), label = species), fontface = "italic", size  = 3, segment.color = NA) +
  xlim(-.05,.2) + ylim(-.33,.1)+
  ...
  theme(panel.background = element_blank(), 
        ...
        legend.position = "none",)
meanvar_biplot
ggsave(meanvar_biplot, filename = "meanvar_biplot_quadXorigin.png", width = 5, height = 5)
